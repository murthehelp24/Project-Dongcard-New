          ...
          <form onSubmit={hdlNotify} className="space-y-6">
            <div className="form-control">
              ...
              <div className="mt-2 flex flex-col items-center gap-4">
                <label className="w-full min-h-40 flex flex-col items-center justify-center border-2 border-dashed border-slate-300 rounded-2xl cursor-pointer hover:bg-slate-50 hover:border-primary transition-all group overflow-hidden">
                  {preview ? (
                    ...
                    <div className="flex flex-col items-center justify-center pt-5 pb-6">
                      ...
                      <p className="text-xs text-slate-400">JPG, PNG (ขนาดไม่เกิน 5MB)</p>
                    </div>
                  )}
            ...
            <button
              type="submit"
              disabled={loading}
              className={`btn btn-primary w-full text-lg font-bold rounded-2xl shadow-lg h-14 transition-all active:scale-95 ${loading ? 'loading' : ''}`}
            >
              {loading ? 'กำลังส่งข้อมูล...' : 'ยืนยันแจ้งชำระเงิน'}
            </button>
          </form>
        </div>
      </div>
